...
class StateFrame:
    ...
    def to_reference_frame(
        ...
        qmat = np.linalg.qr(np.vstack(np.atleast_2d(ax1, ax2, ax3))).Q
        rmat = Rotation.from_matrix(qmat)

        # apply to markers
        if self._markers.shape[0] > 0:
            markers = self._markers.copy() - np.ones_like(self._markers) * ori
            markers.iloc[:, :] = rmat.apply(markers)
        else:
        ...
        if self._forceplatforms.shape[0] > 0:
            ...
            fpcols = fps.columns
            ocols = [i for i, v in enumerate(fpcols) if v[1] == "ORIGIN"]
            fcols = [i for i, v in enumerate(fpcols) if v[1] == "FORCE"]
            tcols = [i for i, v in enumerate(fpcols) if v[1] == "TORQUE"]
            fps.iloc[:, ocols] -= np.ones_like(fps.iloc[:, ocols]) * ori
            for cols in [ocols, fcols, tcols]:
                fps.iloc[:, cols] = rmat.apply(fps.iloc[:, cols])
        else:
